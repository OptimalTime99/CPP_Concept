#include <iostream>
using namespace std;

// TODO: Phone 클래스 구현
// - displayBrand()와 showFeature() 순수 가상 함수를 포함하도록 구현하세요
// - 소멸자를 반드시 virtual로 선언해야 하는 이유: 부모 포인터로 자식 객체를 delete할 때, 자식 소멸자가 반드시 호출되도록 하기 위해서
class Phone {
public:
    virtual void displayBrand() = 0;
    virtual void showFeature() = 0;
    virtual ~Phone() {}
};

// Samsung 클래스 작성 (Phone 클래스를 상속받음)
class Samsung : public Phone {
public:
    void displayBrand() {
        cout << "Samsung" << endl;
    }
    void showFeature() {
        cout << "Galaxy S 시리즈" << endl;
    }
};

// pple 클래스 작성 (Phone 클래스를 상속받음)
class Apple : public Phone {
public:
    void displayBrand() {
        cout << "Apple" << endl;
    }
    void showFeature() {
        cout << "iPhone Pro 시리즈" << endl;
    }
};

// Xiaomi 클래스 작성 (Phone 클래스를 상속받음)
class Xiaomi : public Phone {
public:
    void displayBrand() {
        cout << "Xiaomi" << endl;
    }
    void showFeature() {
        cout << "Redmi Note 시리즈" << endl;
    }
};

int main() {
    // TODO: main 함수 구현
    // - Phone* 타입의 배열을 생성하여 Samsung, Apple, Xiaomi 객체를 저장
    // - 반복문을 사용하여 각 객체의 displayBrand()와 showFeature()를 호출
    // - 반복문을 사용하여 메모리 해제를 위해 delete 호출

    // 힙 메모리 공간 확보 후 각 객체 생성. 
    // 생성된 객체의 시작 주소가 반환되어 포인터 배열의 값이 될 수 있다.
    // 각 파생 클래스의 타입은 기본 클래스의 타입으로 업캐스팅된다.
    Phone* arr[] = { new Samsung(), new Apple(), new Xiaomi()};

    for (Phone* obj : arr) {
        obj->displayBrand();
        obj->showFeature();
    }

    for (Phone* obj : arr) {
        delete obj;
    }
    

    return 0;
}